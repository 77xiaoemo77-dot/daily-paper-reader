...
tags: ["query:q6"]
score: 8.0
evidence: 利用稀疏注意力机制进行金融多通道时间序列预测
tldr: 针对多通道时间序列预测中通道间交互捕捉不足的问题，本文提出Li-Net架构。该模型通过动态压缩序列与通道维度，结合稀疏Top-K注意力机制与多尺度投影，有效捕捉线性及非线性依赖关系。Li-Net还融合了多模态嵌入以引导注意力，在保持高预测精度的同时，显著降低了内存占用并提升了推理速度。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2603.18712v1/fig-001.webp\", \"caption\": \"Figure 2: Memory usage and efficiency\", \"page\": 11, \"index\": 1, \"width\": 908, \"height\": 458}, {\"url\": \"assets/figures/arxiv/2603.18712v1/fig-002.webp\", \"caption\": \"Figure 3: Parameter Analysis\", \"page\": 12, \"index\": 2, \"width\": 827, \"height\": 467}]"
motivation: 传统多通道预测方法往往忽视了通道间的复杂动态交互，难以平衡预测精度与计算效率。
method: 提出Li-Net架构，利用稀疏Top-K Softmax注意力机制、多尺度投影及多模态信息融合来捕捉跨维度依赖。
result: 在多个真实数据集上的实验表明，Li-Net在达到SOTA性能的同时，具有更低的内存消耗和更快的推理速度。
conclusion: Li-Net通过创新的稀疏注意力和非线性模块设计，为高效且准确的多变量时间序列预测提供了一种优越的解决方案。
---

## 摘要
多通道时间序列预测任务在金融、供应链管理和能源规划等众多领域无处不在。有效捕捉通道内和通道间的复杂动态依赖关系对于实现准确预测至关重要。然而，传统方法很少关注通道间相互作用的学习。本文提出了线性网络（Li-Net），这是一种专为多通道时间序列预测设计的新型架构，能够捕捉通道间的线性和非线性依赖关系。Li-Net 在序列和通道维度上动态压缩表示，通过可配置的非线性模块处理信息，并随后重建预测结果。此外，Li-Net 在多尺度投影框架中集成了稀疏 Top-K Softmax 注意力机制以应对这些挑战。其核心创新在于能够无缝整合并融合多模态嵌入，引导稀疏注意力过程聚焦于最具信息量的时间步和特征通道。在多个真实世界基准数据集上的实验结果表明，与最先进的基准方法相比，Li-Net 取得了具有竞争力的性能。此外，Li-Net 在预测准确性与计算负担之间实现了卓越的平衡，表现出显著更低的内存占用和更快的推理速度。详细的消融研究和参数敏感性分析验证了所提架构中各关键组件的有效性。关键词：多元时间序列预测、稀疏注意力机制、多模态信息融合、非线性关系。
